...
- 깊이에 따른 공 크기·가림·명암을 표현하는 WebGL 3D 연출
...
4. `2D` 또는 `3D` 렌더링을 선택합니다. 기본값은 2D이며 추첨 중에도 결과를 유지한 채 바꿀 수 있습니다.
5. 필요하면 효과음 토글을 켭니다. 저장된 선택이 없을 때의 기본값은 음소거입니다.
6. `추첨 시작`을 누릅니다.
7. 수동 모드에서는 공이 계속 섞이며, `다음 공 뽑기`를 누를 때마다 한 공이 나옵니다.
8. 자동 모드에서는 카운트다운 없이 3~7초 사이에 공이 하나씩 나옵니다.
9. 목표한 개수의 추첨이 끝나면 `결과 복사`로 순서 목록을 복사할 수 있습니다.
10. `처음부터 다시`를 누르면 이름 목록과 현재 설정을 유지한 채 설정 화면으로 돌아갑니다.
...
현재 기준으로 9개 테스트 파일의 79개 테스트가 통과합니다. 주요 검증 범위는 다음과 같습니다.
...
- 2D 기본값, 3D 원근 투영, 설정 저장·복원과 추첨 중 렌더링 전환
...
- 공은 최대 45개까지 지원합니다.
- 3D 모드는 WebGL 빌보드 텍스처와 원근 투영을 사용하며 메시 기반 구체 회전·충돌을 계산하는 고급 3D 엔진은 아닙니다.

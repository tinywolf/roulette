...
4. 필요하면 효과음 토글을 켭니다. 저장된 선택이 없을 때의 기본값은 음소거입니다.
5. `추첨 시작`을 누릅니다.
...
`모두 지우기`는 입력란과 브라우저에 저장된 이름 목록을 함께 삭제하되 선택한 옵션은 유지합니다. 페이지를 새로고침하면 이름 목록, 전체/일부 추첨, 추첨 개수, 진행 방식과 효과음 설정은 복원되며 진행 결과와 순서는 초기화됩니다.
...
현재 기준으로 9개 테스트 파일의 75개 테스트가 통과합니다. 주요 검증 범위는 다음과 같습니다.

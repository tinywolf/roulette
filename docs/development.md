...
2026-07-28 기준 전체 8개 테스트 파일, 50개 테스트와 정적 빌드가 통과했고 프로덕션 의존성 취약점은 0건이었다.
...
`LotteryMachine`은 `remainingBalls`, `isMixing`, `visualBall`을 받아 그린다. `lotteryMotion.ts`의 3축 좌표·속도·투영 결과를 `DrawEngine`에 되돌려 보내지 않는다. 이 원칙을 깨면 저성능 기기에서 추첨 결과가 달라질 수 있다.
...
- 공 간 평면 충돌 해소를 다시 추가하면 투영 겹침과 45개 혼합 유동성이 줄어들 수 있다.
- 깊이 원근 범위를 바꾸면 모바일에서 이름 가독성과 원형 경계 침범을 함께 확인한다.
...
- 45개 혼합 프레임에서 공의 화면상 겹침·깊이 크기 차이와 역동적인 위치 변화가 보이는가

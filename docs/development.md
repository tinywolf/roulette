...
2026-07-28 기준 전체 7개 테스트 파일, 38개 테스트와 정적 빌드가 통과했고 프로덕션 의존성 취약점은 0건이었다.
...
`LotteryMachine`은 `remainingBalls`, `isMixing`, `visualBall`을 받아 그린다. Canvas 좌표나 충돌 결과를 `DrawEngine`에 되돌려 보내지 않는다. 이 원칙을 깨면 저성능 기기에서 추첨 결과가 달라질 수 있다.
...
`LotteryMachine` 내부의 노드·렌더링만 변경하고 `DrawSession`을 수정하지 않는다. 애니메이션 루프에서 발생한 예외는 `onError`로 격리하고, 리사이즈 후에도 세션과 공 ID를 보존한다.
...
문서 작성 시 이 명령을 실제 실행했고 1개 파일의 4개 테스트가 통과했다.
...
- 0과 5 인덱스가 각각 5초와 10초 간격을 만든다.
- 난수 소스 예외가 `SecureRandomError`로 바뀐다.

### 타이머 테스트

React 통합 테스트에서는 `vi.useFakeTimers()`와 `vi.setSystemTime()`을 사용한다. 수동 혼합은 2,400ms, 자동 첫 결과는 주입된 난수에 따라 5,000ms 경계 전후를 검증한다. 테스트 종료 전 `vi.useRealTimers()`로 복구한다.
...
- 로컬 Git 저장소의 초기 커밋 전이므로 문서 revision은 `unversioned`다. 첫 커밋 이후 문서를 갱신할 때 실제 revision으로 교체한다.
- 최대 공 개수는 45개다.
- Canvas는 실제 물리 엔진이 아닌 가벼운 충돌 연출이다.

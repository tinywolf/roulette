...
일반 입력은 줄바꿈·콤마로 나눈 뒤 각 항목 끝의 ASCII `*반복횟수`를 확장한다. `*`는 예약 문자이고 한 항목에 한 번만 사용할 수 있으며, 횟수는 1~45 사이 정수다. 이름 길이는 `*횟수`를 제외한 값으로, 전체 개수는 모든 반복식을 확장한 결과로 검증한다. 숫자 범위 문법과 반복식은 함께 사용할 수 없다.

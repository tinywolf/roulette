...
| P1 | 자연스러운 SVG 회전, 후보 전체 이름, 복사·이미지 저장, 직접 진입·뒤로 가기, 동작 감소, 반응형·키보드 |
...
- 회전 중 회전 버튼과 결과 이미지 저장을 비활성화한다.
- 준비 상태에서 `Space`는 회전을 시작하고, 결과 이력이 있는 준비 상태에서 `R`은 재추첨을 위해 결과 이력만 비운다.
- 회전·재추첨 버튼은 `aria-keyshortcuts`와 화면에 보이는 `Space`·`R` 힌트를 함께 제공한다.
- 단축키는 키 반복·문자 조합·보조키 조합을 무시하고, 입력 요소를 조작하거나 회전 중일 때는 동작하지 않는다.
...
- `결과 복사`와 `이미지 저장`을 제공하고 결과 영역의 `결과 비우기` 버튼은 제거한다.
- 이미지 저장은 결과 이력이 하나 이상 있고 회전 중이 아닐 때 현재 결과 카드만 고해상도 PNG로 내려받는다.
- 저장 이미지에는 제목·결과 수·전체 결과 순서와 이름을 포함하고 돌림판·후보 목록·조작 버튼·상태 메시지는 제외한다.
- 화면에서 스크롤되는 결과도 저장 이미지에는 모두 포함하며 파일명에는 저장 시각을 포함한다.
- 이미지 생성·다운로드 실패는 결과를 유지하고 한국어 상태 메시지로 알린다.
- `R` 재추첨은 기존처럼 결과 이력만 비우고 후보·설정·현재 회전각을 유지한다.
- 복사 텍스트는 다음 형식이다.
...
- 키보드로 선택, 입력, `Space` 회전, `R` 재추첨, 복사, 비우기와 화면 전환을 수행할 수 있다.
...
- 결과 복사 형식과 결과 전용 PNG 이미지 저장·실패 격리
- 후보·옵션 저장·복원·삭제·실패
- 효과음 기본 음소거와 수명주기 정리
...
- 결과 복사·이미지 저장과 기능별 저장 수명주기가 요구사항대로 동작한다.

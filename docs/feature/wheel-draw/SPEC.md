...
- 결과를 회전 전에 안전한 난수로 확정하고 약 4초의 자연스러운 회전으로 표현한다.
...
- 결과 선확정과 목표 구획 정지
- 약 4초 회전, 동작 감소 대응과 테스트 후 감각 미세 조정
...
- 조정은 약 4초, 최소 6회 순방향 회전, 목표 구획 중심 정지와 결과 선확정 계약을 바꾸지 않는다.
...
- 일반 회전이 약 4초 동안 역회전·점프 없이 자연스럽게 감속한다.

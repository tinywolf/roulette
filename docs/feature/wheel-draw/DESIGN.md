...
type WheelSession = {
  ...
  phase: "ready" | "spinning" | "completed" | "error";
...
다음 항목은 구조 결정과 독립적이며 feature SPEC 작성 시 확정한다.

- 돌림판의 기본 추첨 횟수와 최대 회전 횟수
- 수동 회전만 제공할지 자동 연속 회전도 제공할지
- 당첨 후보 제거 옵션을 제공할지, 항상 복원 추첨으로 제한할지
- 후보 수보다 많은 추첨 횟수를 허용할지
- 회전 시간, 최소 회전 수와 easing
- 구획 레이블 축약 규칙과 모바일 표현
- 돌림판 전용 효과음과 음소거 기본값
- 결과 복사·이미지 저장·진행 결과 저장 범위
- 추첨기 선택 화면의 직접 URL과 브라우저 뒤로 가기 계약

...
- 3D 모드는 WebGL 텍스처 공을 원근 투영하고 깊이에 따라 크기와 가림 순서를 바꾸며 명암과 구형 가이드로 입체감을 표현한다.
...
- WebGL 3D 모드는 빌보드 텍스처와 원근 투영을 사용하며 메시 기반 구체 충돌·카메라 물리까지 계산하는 고급 3D 엔진은 아니다. 더 정교한 광원·구체 회전·충돌이 필요하면 별도 엔진 도입을 판단한다.

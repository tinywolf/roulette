...
| 향후 | 초기 범위 밖 | 46개 이상 공 확장, 물리 연출 개선, 배포 자동화, 접근성, 모니터링 |
...
- 각 공의 추첨 간격은 5~10초 사이의 정수 초를 암호학적 난수로 균등하게 선택한다.
- 첫 공에도 동일한 5~10초 대기 시간을 적용한다.
...
- `createAutoSchedule`은 목표 공 ID 각각의 사이에 5~10초의 균등 무작위 간격을 누적한다.
...
- 원형 용기 경계와 공의 이동·반사·간단한 공 간 충돌을 시각적으로 자연스럽게 흉내 낸다.
...
- 46개 이상 공 확장이나 물리 엔진 도입은 테스트 결과를 근거로 별도 판단한다.
...
- 자동 모드에서 카운트다운 없이 5~10초 사이에 공이 배출되는지 확인한다.
...
- Canvas 2D의 단순 충돌 연출이 기대한 로또 추첨기 느낌에 미치지 못할 수 있다. 초기 구현 후 시각 테스트를 거쳐 개선 또는 물리 엔진 도입을 판단한다.
- 백그라운드 탭의 실제 실행 빈도는 브라우저·운영체제 정책에 따라 달라진다. 절대 시각 기반 복구로 논리적 일관성은 보장하지만 보이지 않는 애니메이션 실행은 보장하지 않는다.
- 공 개수를 45개보다 늘리면 가독성, 충돌 계산량, 모바일 배치가 달라지므로 별도 성능·UX 검증이 필요하다.

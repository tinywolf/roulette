...
- `재추첨`이 수동 혼합이나 자동 진행 중 실행되면 기존 애니메이션·예약 작업을 취소하고 같은 후보와 설정의 새 세션만 진행한다.
...
- 수동·자동 진행 및 완료 상태의 재추첨 결과·일정 초기화와 현재 설정 유지
...
- 수동 혼합·자동 진행·완료 화면에서 재추첨하면 설정 화면을 거치지 않고 결과가 0개인 새 세션으로 시작하는지 확인한다.

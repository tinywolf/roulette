...
- 서버 없이 정적 웹앱으로 동작하고, 입력 목록과 사용자가 선택한 설정 옵션만 브라우저에 보존한다.
...
- 새로고침하거나 설정으로 돌아가면 추첨 진행 상태는 초기화되고 입력 목록과 설정 옵션은 유지된다.
...
- `이름*반복횟수` 형식의 중복 공 축약 입력
- 입력 검증 및 오류 표시
- 중복 이름을 서로 다른 공으로 처리
- 수동·자동 추첨 모드
- 전체 또는 지정한 개수의 비복원 순차 추첨
- 선택 가능한 Canvas 2D·WebGL 3D 공 섞임과 배출 연출
- 추첨 순서 누적 표시와 결과 복사
- 입력 목록의 브라우저 저장
- 효과음 음소거/활성화 토글
- 모바일·데스크톱 반응형 UI
...
| P1 | 초기 경험 필수 | Canvas 2D·WebGL 3D 선택 연출, 반응형 UI, 결과 복사, 백그라운드 경과 상태 복구 |
| P2 | 품질 보강 | 효과음 토글, 저장·복사 실패 안내, 별도 데스크톱 스모크 테스트 |
| 향후 | 초기 범위 밖 | 46개 이상 공 확장, 메시 기반 고급 3D 물리 엔진, 배포 자동화, 접근성, 모니터링 |
...
- 일반 이름 또는 숫자 뒤에 ASCII 별표 `*`와 반복 횟수를 붙이면 해당 값을 반복 횟수만큼 확장한다.
- `민지*2, 준호*3`은 `"민지"` 두 개와 `"준호"` 세 개로, `7*2`는 `"7"` 두 개로 해석한다.
- 반복식의 `*` 앞뒤 공백은 허용하며 반복 횟수는 1~45 사이의 정수여야 한다.
- `*`는 반복식 전용 예약 문자이며 공 이름 자체에 포함하거나 한 항목에서 여러 번 사용할 수 없다.
- 줄바꿈·콤마로 나눈 각 항목이 `시작숫자~끝숫자` 형식이면 시작부터 끝까지의 정수를 오름차순 문자열 이름으로 확장한다.
- `1~45`는 `"1"`부터 `"45"`까지 45개 이름으로 해석한다.
- 숫자 범위에는 ASCII 물결표 `~`를 사용하며 앞뒤 공백은 허용한다.
- 숫자 범위는 일반 이름·숫자·반복식·다른 범위와 같은 목록에 함께 입력할 수 있다.
- `1~3, 민지*2, 7`은 `"1"`, `"2"`, `"3"`, `"민지"`, `"민지"`, `"7"`로 해석한다.
- 숫자 범위 자체에는 `*` 반복을 적용하지 않는다.
- 범위의 선행 0은 제거한다. 예를 들어 `01~03`은 `"1"`, `"2"`, `"3"`으로 해석한다.
- 입력 목록은 사용자가 수정할 때 브라우저 저장소에 보존한다.
- `입력 비우기`는 입력창과 브라우저에 저장된 이름 목록을 함께 삭제한다.

### 5.2 입력 검증

- 유효한 이름 개수는 최소 2개, 최대 45개다.
- 숫자 범위의 시작과 끝은 0 이상의 안전한 정수이며 시작 숫자는 끝 숫자보다 클 수 없다.
- 개별 숫자 범위는 한 개 이상의 값을 만들 수 있으며, 모든 항목과 반복식을 확장한 최종 공 개수는 최소 2개, 최대 45개다.
- 반복식을 모두 확장한 뒤의 공 개수가 최소 2개, 최대 45개여야 한다.
- 이름 하나의 최대 길이는 20자다.
- 반복 횟수는 이름 길이에 포함하지 않으며 확장 전 이름을 기준으로 20자 제한을 검증한다.
...
- 설정 화면에서 추첨기 렌더링 모드를 2D 또는 3D로 선택하며 기본값은 3D다.
- 추첨 시작 후 이름 입력, 추첨 개수와 모드 선택을 비활성화한다.
- 렌더링 모드는 추첨 상태와 독립적인 보기 옵션이므로 추첨 중에도 변경할 수 있다.
...
- `재추첨`을 누르면 설정 화면으로 돌아가지 않고 현재 후보 공, 추첨 개수, 진행 방식과 보기·효과음 설정을 그대로 사용해 새 추첨 세션을 시작한다.
- 추첨 화면에서는 `R` 키로도 재추첨할 수 있으며 키 반복, 한글 조합 중 입력과 `Ctrl`·`Command`·`Alt`·`Shift` 조합은 무시한다.
- 수동 준비·혼합 또는 자동 진행 중 버튼이나 `R` 키로 재추첨을 요청하면 현재 진행을 계속할지 재추첨할지 한 번 확인한다.
- 진행 중 재추첨 확인에서 취소하면 결과·남은 공·일정과 현재 진행을 그대로 유지하고, 확인하면 새 추첨 세션을 시작한다.
- 자동 추첨은 재추첨 확인을 표시하는 동안에도 일시정지하지 않는다.
- 완료 또는 오류 상태의 재추첨은 별도 확인 없이 즉시 새 추첨 세션을 시작한다.
- 재추첨은 이전 결과와 남은 공, 수동 선택 대기·진행 중 애니메이션 또는 자동 일정을 모두 버리고 새 난수 선택과 일정으로 처음부터 진행한다.
- 수동 재추첨은 즉시 준비 상태가 되고 자동 재추첨은 새 일정으로 즉시 자동 진행을 시작한다.
- `처음부터 다시`와 완료·오류 상태의 재추첨에는 확인창을 표시하지 않는다.
...
- 수동 추첨을 시작하면 첫 공 선택 전부터 목표 개수에 도달할 때까지 추첨기 안의 공을 계속 섞는다.
- 사용자가 `다음 공 뽑기` 버튼 또는 `Space` 키를 누르면 버튼을 일시적으로 비활성화한다.
- `Space` 단축키는 수동 추첨의 준비 상태에서만 동작하며, 키 반복 입력과 다른 조작 요소에 포커스가 있는 경우에는 무시한다.
- 상시 혼합을 유지한 채 약 2~3초 후 선택된 공을 배출한다.
- 배출 연출과 결과 반영이 끝난 후 목표 개수가 남아 있으면 버튼을 다시 활성화하고 공은 계속 섞는다.
...
- 추첨기가 공을 섞는 동안에는 공기 흐름을 연상시키는 낮고 연속적인 바람 소리를 재생한다.
- 혼합 중 남은 공이 2개 이상이면 플라스틱 공끼리 부딪히는 짧은 충돌음을 불규칙한 간격으로 바람 소리 위에 겹쳐 재생한다.
- 충돌음의 발생 간격은 남은 공 수가 많을수록 조밀하고 적을수록 성기게 조정하며, 공이 1개 이하이면 충돌음을 재생하지 않는다. 이 연출은 추첨 결과·난수·자동 일정에 영향을 주지 않는다.
- 수동 모드의 준비·혼합 상태와 자동 모드의 진행 상태에서 혼합 효과음을 유지한다.
- 추첨 완료 후 남은 공이 중력으로 정착하는 경우 바람음은 약 2초 동안 음량과 주파수를 낮추며 감쇠하고, 남은 공이 2개 이상이면 음량과 빈도가 감소하는 잔여 충돌음을 짧게 재생한다.
- 남은 공 없이 완료되거나 오류·설정 화면 전환·음소거가 발생하면 혼합 효과음을 빠르게 정지한다.
- 저장된 설정이 없는 최초 상태는 음소거다.
- 사용자는 화면의 토글로 효과음을 활성화하거나 다시 음소거할 수 있다.
- 효과음 선택 상태는 다른 설정 옵션과 함께 브라우저에 저장하고 새로고침 후 복원한다.
- 소리 재생 실패는 추첨을 중단시키지 않는다.

### 5.9 렌더링 모드

- 기존 Canvas 2D 렌더링과 WebGL 3D 렌더링을 모두 제공한다.
- 2D와 3D 선택 토글은 설정 화면과 추첨 화면 모두 효과음 토글과 같은 제어 영역에 배치한다.
- 2D 모드는 깊이와 관계없이 같은 공 크기를 유지하는 기존 구형 투영 연출을 그대로 사용한다.
- 3D 모드는 같은 3축 운동 상태를 원근 투영하고, 깊이에 따른 크기·가림·명암으로 입체감을 표현한다.
- 3D 공에도 이름을 함께 표시하고 공간이 부족한 이름은 말줄임표로 표시한다.
- 정상 3D 경로는 유리구·공·받침·그림자를 하나의 WebGL Canvas에서 합성한다.
- 유리구와 받침의 정적 장면 텍스처는 리사이즈 때만 갱신하고, 매 프레임에는 공 위치와 배출 상태만 갱신한다.
- 추첨 중 렌더링 모드를 바꾸면 현재 공 위치를 이어받아 화면만 다시 그리며 추첨 세션·결과·자동 일정은 변경하지 않는다.
- 일부 추첨 완료 후 남은 공의 중력 정착은 두 렌더링 모드 모두 같은 3축 물리 상태를 사용한다.
- 정착 중 느린 공은 구형 바닥이나 아래쪽 공에 지지된 경우에만 수면 상태로 전환하며, 공중에서는 중력 낙하를 계속한다.
- 정착 시작 3초 이후에는 반발과 속도 감쇠를 점진적으로 강화하고, 4초 이후에는 현재 속도를 이어받는 1초 수렴 전환으로 마지막 중력·충돌 보정 위치까지 이동해 5초 하드 리밋에 모든 공을 고정한다.
- WebGL을 사용할 수 없거나 3D 렌더링이 실패하면 추첨을 중단하지 않고 경고를 표시한 뒤 Canvas 기반 대체 연출을 사용한다.

### 5.10 저장과 초기화

- 이름 입력 목록과 설정 옵션을 `localStorage`에 저장한다.
- 저장 값은 `{ version: 1, rawInput: string }` 형식으로 입력 원문과 스키마 버전을 포함한다.
- 설정 옵션은 추첨 방식, 전체/일부 추첨 선택, 일부 추첨 개수, 효과음 상태와 렌더링 모드를 버전이 포함된 별도 값으로 저장한다.
- 남은 공, 뽑힌 순서, 자동 추첨 일정과 완료 상태는 저장하지 않는다.
- 새로고침하면 저장된 입력 목록과 설정 옵션을 복원하되 추첨 진행 상태는 초기화하고 설정 화면에서 새 추첨을 시작한다.
- `처음부터 다시`는 현재 입력과 설정 옵션을 유지한 채 설정 화면으로 돌아간다.
- `입력 비우기`는 입력 목록만 삭제하며 선택한 설정 옵션은 유지한다.
- 저장 실패 시 현재 세션의 입력과 추첨 기능은 유지하고 `목록을 저장하지 못했습니다`라는 경고를 표시한다.
- 설정 저장·복원 실패 시 기본 설정을 사용해 앱을 계속 실행하고 경고를 표시한다.
...
- `민지*2, 준호*3`은 다섯 개의 공으로 확장되며 같은 이름의 공도 서로 다른 ID를 가진다.
- `7*2`는 이름이 `"7"`인 서로 다른 두 공으로 확장된다.
- `1~3, 민지*2, 7`은 숫자 범위와 일반·반복 항목을 입력 순서대로 합친 여섯 개의 공으로 확장된다.
- `1~1, 민지`는 `"1"`과 `"민지"` 두 공으로 확장된다.
- `민지*0`, `민지*46`, `민지*1.5`, `민지*둘`, `*2`, `민지**2`는 오류를 표시하고 시작하지 않는다.
- `민지,, 준호`의 빈 항목은 무시하고 두 개의 공으로 처리한다.
- `1~45`는 숫자 이름을 가진 45개의 서로 다른 공으로 생성한다.
- `민지~준호`, `1~3~5`, `1~3*2`, `3~1`, `1~1`, `1~46`은 오류를 표시하고 시작하지 않는다.
...
- 수동 혼합이나 자동 진행 중 `재추첨` 확인을 취소하면 기존 진행을 유지하고, 확인하면 기존 애니메이션·예약 작업을 취소한 뒤 같은 후보와 설정의 새 세션만 진행한다.
...
- 언어: TypeScript
- 빌드 도구: Vite
- 렌더링: React DOM + HTML Canvas 2D + WebGL
...
- `SetupPanel`: 이름·추첨 개수 입력, 검증 결과, 추첨·렌더링 모드 선택, 시작, 입력 비우기
- `RenderModeToggle`: 설정·추첨 화면의 2D/3D 보기 전환
- `LotteryMachine`: Canvas 2D·WebGL 생성, 크기 조정, 공 움직임·배출 연출과 렌더러 전환
- `DrawControls`: 다음 공 뽑기, 현재 설정으로 재추첨, 처음부터 다시
...
saveSetupOptions(options: SetupOptions): {
  value: null;
  warning: string | null;
}
```
...
- `parseNames`는 줄바꿈·콤마로 항목을 나누고 각 항목의 단일 `*반복횟수` 접미사를 분리한다. 접미사를 제외한 항목이 `~`를 포함하면 숫자 범위로, 아니면 일반 값으로 해석한 뒤 반복하고 최종 개수와 이름 길이를 검증한다.
...
- `loadSetupOptions`는 저장값의 버전과 타입을 검증하고 손상 시 기본 설정과 경고를 반환한다.
- `saveSetupOptions`는 추첨 세션을 제외한 사용자 설정만 저장하고 실패를 경고 값으로 반환한다.

### 7.5 2D·3D 연출

- 운동 계산은 별도 물리 엔진 없이 공통 3축 상태와 `requestAnimationFrame`을 사용한다.
- 각 공은 가상 `x`, `y`, `z` 위치와 3축 속도를 가지며 구형 공간 경계에서 반사한다.
- 혼합 중에는 여러 회전축의 흐름과 시간·공별 위상이 다른 난류를 적용해 방향과 깊이를 지속해서 바꾼다.
- 수동 모드는 `ready`와 `mixing` 상태 모두 같은 혼합 운동을 사용하고, 버튼 상태와 Canvas 운동 상태를 분리한다.
...
- 2D 모드는 공을 가상 깊이순으로 정렬해 뒤쪽부터 그리되 모든 공을 같은 크기로 렌더링하고 투명도와 그리기 순서만으로 앞뒤를 구분한다.
- 3D 모드는 WebGL 텍스처 공을 원근 투영하고 깊이에 따라 크기와 가림 순서를 바꾸며, 외곽선 없는 유리구·면 반사·3D 받침과 함께 하나의 장면으로 합성한다.
- 정적 장면의 배경·전경과 깊이순 공·배출 공은 한 버텍스 버퍼에 순서대로 기록해 프레임당 한 번의 draw call로 그린다.
- WebGL 텍스처는 사용자 입력을 Canvas 텍스트로만 그리며 HTML로 해석하지 않는다.
- 두 모드는 같은 운동 노드와 공 ID를 공유해 추첨 중 전환에도 움직임이 이어진다.
...
- 일반 정착 판정을 통과하지 못하더라도 활성 렌더링 기준 약 5초 안에 최종 위치를 확정하고 렌더링 루프를 중단한다. 백그라운드 탭처럼 프레임이 중단된 경우에는 복귀 직후 순간 이동하지 않고 마지막 수렴 전환을 화면에 표시한다.
...
- 46개 이상 공 확장이나 메시 기반 고급 3D 물리 엔진 도입은 테스트 결과를 근거로 별도 판단한다.
...
| WebGL 생성·렌더링 실패 | 경고 표시, Canvas 대체 연출로 계속 진행 |
...
- 엄격한 FPS 목표는 두지 않는다.
- 저성능 기기에서 Canvas 또는 WebGL 프레임이 낮아져도 추첨 순서, 제거, 완료 상태는 정확해야 한다.
- 화면 크기나 렌더링 모드 변경 시 추첨기를 재배치하되 추첨 세션은 유지한다.
...
- 이름은 외부 서버로 전송하지 않는다.
- 이름은 사용자의 브라우저 `localStorage`에 평문으로 저장되므로 공용 기기에서는 `입력 비우기`로 삭제할 수 있게 한다.
...
- 줄바꿈·콤마 혼합 입력 파싱
- 이름·숫자 반복식의 확장, 공백 허용, 형식·횟수·전체 개수 경계 검증
- 숫자 범위의 오름차순 확장, 일반·반복 항목과의 조합, 범위 반복 차단, 역순·개수 경계 검증
- 앞뒤 공백 제거와 빈 항목 무시
- 최소·최대 개수 및 20자 길이 검증
- 중복 이름을 별도 ID의 공으로 생성
- 전체·일부 비복원 추첨과 추첨 개수 경계
- 난수 실패 시 추첨 중단
- 수동 모드의 중복 클릭 방지와 `Space` 단축키의 상태·반복 입력 제한
- 수동·자동 진행 상태의 버튼·`R` 재추첨 확인과 취소 시 상태 유지, 완료 상태의 즉시 재추첨
- 혼합 상태의 바람 루프·불규칙 충돌음 생성, 남은 공 수별 충돌 밀도와 중복 시작 방지
- 음소거·완료·오류·설정 복귀·앱 종료 시 혼합 음향 노드와 타이머 정리
- 자동 일정의 3~7초 범위와 누적 시각
- 백그라운드 경과 후 일정 복구의 멱등성
- `처음부터 다시`와 새로고침의 입력·설정 유지 및 추첨 상태 초기화
- 설정 옵션 저장·복원, 손상 데이터 기본값 대체와 저장 실패 경고
- 렌더링 모드 기본값·저장·복원과 추첨 중 2D/3D 전환
- 저장·복사 실패 분기
- 결과 복사 문자열 형식
...
- 수동 혼합·자동 진행 중 재추첨은 확인 후 결과가 0개인 새 세션으로 시작하고 취소하면 기존 진행을 유지하며, 완료 화면에서는 확인 없이 즉시 시작하는지 확인한다.
...
- 공중의 저속 공이 조기에 잠들지 않고, 3초 이후 감쇠 강화와 하드 리밋 직전 1초 수렴 전환이 동작하는지 확인한다.
- 추첨 전 선택한 2D/3D 모드가 시작 화면에 적용되고 추첨 중 전환해도 결과·남은 공·일정이 유지되는지 확인한다.
- 3D 모드에서 깊이에 따른 공 크기·가림·명암과 이름 표시가 보이며 WebGL 실패 시 대체 연출로 계속 진행하는지 확인한다.
...
- WebGL 3D 모드는 단일 장면 합성과 빌보드 원근 투영을 사용하며 메시 기반 구체 충돌·카메라 물리까지 계산하는 고급 3D 엔진은 아니다. 더 정교한 광원·구체 회전·충돌이 필요하면 별도 엔진 도입을 판단한다.

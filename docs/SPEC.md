...
- 정착 시작 3초 이후에는 반발과 속도 감쇠를 점진적으로 강화하고, 5초 이후에는 현재 속도를 이어받는 1초 수렴 전환으로 마지막 중력·충돌 보정 위치까지 이동한 뒤 모든 공을 고정한다.
...
- 일반 정착 판정을 통과하지 못하더라도 활성 렌더링 기준 약 6초 안에 최종 위치를 확정하고 렌더링 루프를 중단한다. 백그라운드 탭처럼 프레임이 중단된 경우에는 복귀 직후 순간 이동하지 않고 마지막 수렴 전환을 화면에 표시한다.

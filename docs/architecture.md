...
모든 공의 프레임 간 이동이 0.08 CSS px 이하인 상태가 18프레임 이어지면 마지막 화면을 유지한 채 `requestAnimationFrame`을 중단한다. 3초가 지나면 감쇠·마찰과 수면 임계값을 점진적으로 강화한다. 5초에 도달하면 복사한 운동 노드에서 고감쇠 중력과 추가 겹침 보정으로 최종 위치를 미리 계산하고, 실제 노드는 현재 위치와 속도를 시작값으로 삼는 3차 보간을 통해 1초 동안 목표에 수렴한 뒤 정지한다. 전환 시간은 렌더링 프레임을 기준으로 계산하므로 백그라운드 탭에서 프레임이 중단되어도 복귀 시 공이 순간 이동하지 않는다. 남은 공과 배출 공이 없을 때도 첫 정적 프레임 뒤 중단하며, 상태·공·크기가 바뀌면 즉시 재개한다.

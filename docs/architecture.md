...
`App`은 Canvas와 같은 `shouldMixMachine` 판정을 사용해 효과음이 활성화된 수동 `ready`·`mixing` 또는 자동 `running` 상태에서 `SoundController.startMixing(remainingBallCount)`을 호출한다. 남은 공이 있는 정상 완료는 Canvas의 정착 상태와 함께 `finishMixing(remainingBallCount)`으로 전환하고, 남은 공 없는 완료·오류·설정 복귀와 음소거는 `stopMixing()`으로 전환한다. 공 하나가 빠질 때마다 남은 공 수만 갱신하고 이미 실행 중인 바람 루프를 다시 만들지 않는다.

`SoundController`는 저주파 노이즈 버퍼를 필터링한 바람음을 하나의 반복 `AudioBufferSourceNode`로 유지한다. 바람·충돌 노이즈 버퍼는 같은 `AudioContext` 수명 동안 재사용한다. 짧은 노이즈와 삼각파 공명을 조합한 충돌음은 별도 타이머로 불규칙하게 생성하며, 남은 공이 많을수록 평균 간격을 줄인다. 남은 공이 1개 이하이면 예약된 충돌 타이머를 취소하고 2개 이상으로 늘어나면 다시 예약한다.

정상 완료 후 남은 공이 있으면 기존 바람 소스의 게인을 첫 350ms에 70%까지 낮춘 뒤 약 2초까지 선형 감쇠하고, 저역 필터는 920Hz에서 560Hz로 낮춘다. 공 수에 따라 최대 세 번의 약해지는 잔여 충돌음을 재생한 뒤 소스를 정리한다. 감쇠 도중 재추첨하면 예약된 정지를 취소하고 같은 바람 소스를 정상 출력으로 되돌린다. 각 충돌이 끝나면 해당 소스·필터·공명·게인 노드 연결을 즉시 해제해 장시간 혼합의 GC 부담을 제한한다. 음향 타이머에서 사용하는 `Math.random()`은 음향 변화에만 쓰고 `DrawEngine`, Web Crypto 난수와 자동 일정에는 전달하지 않는다.
...
일부 추첨 완료 시 `App`은 남은 공이 있는 경우에만 `isSettling`을 전달한다. `LotteryMachine`은 혼합 운동 대신 아래 방향 중력과 구형 벽 반발을 적용하고, 3회 반복하는 공 간 위치 보정·저반발 충돌·마찰로 남은 공을 바닥에 쌓는다. 저속 공은 구형 바닥이나 아래쪽 공에 실제로 지지된 경우에만 수면 상태로 전환해 공중에서 속도가 매 프레임 초기화되는 느린 낙하를 방지한다.

모든 공의 프레임 간 이동이 0.08 CSS px 이하인 상태가 18프레임 이어지면 마지막 화면을 유지한 채 `requestAnimationFrame`을 중단한다. 3초가 지나면 감쇠·마찰과 수면 임계값을 점진적으로 강화한다. 4초에 도달하면 복사한 운동 노드에서 고감쇠 중력과 추가 겹침 보정으로 최종 위치를 미리 계산하고, 실제 노드는 현재 위치와 속도를 시작값으로 삼는 3차 보간을 통해 1초 동안 목표에 수렴해 5초 하드 리밋에 정지한다. 전환 시간은 렌더링 프레임을 기준으로 계산하므로 백그라운드 탭에서 프레임이 중단되어도 복귀 시 공이 순간 이동하지 않는다. 남은 공과 배출 공이 없을 때도 첫 정적 프레임 뒤 중단하며, 상태·공·크기가 바뀌면 즉시 재개한다.
